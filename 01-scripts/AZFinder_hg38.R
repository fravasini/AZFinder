...
cat("\n ##")
cat("\n #")
cat(paste0("\n # [",Sys.time(),"] ","Running AZFinder for hg38 reference\n"))
cat("\n")
...
commA = paste0("samtools depth -ab ",pywd,"/02-files/AmpliconPositions_hg38 -f ",bamlist," > amplicon_depth.txt")
system(commA, ignore.stderr=T)

cat("\nObtaining depth values for normalization region...\n")
commB = paste0("samtools depth -ab ",pywd,"/02-files/NormalizationRegion_hg38.bed -f ",bamlist," > Norm_depth.txt")
...
# read sample list
#list<-scan(bamlist,character(),quiet=T)
